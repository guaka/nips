...
with the [NIP-44](44.md) HKDF function and salt set to "default", concatenating it with the to-be-obfuscated value,
...
```js
{
  ...
  "pubkey": "<me>",
  "tags": [
    ["d", toHex(sha256(hkdf("<my-private-key>", salt: "default") || "<another-user-pubkey>"))]
    ["n", "example-set-name"]
  ],
  "content": nip44Encrypt(JSON.stringify([
    ["d", "<another-user-pubkey>"]
...
| denotification | (usually own) notes not monitored for new replies, zaps and reactions |
...
| notification | (usually not own) notes monitored for new replies, zaps and reactions |
